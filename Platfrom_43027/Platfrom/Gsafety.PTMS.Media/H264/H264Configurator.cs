    ...
    sealed class H264Configurator : VideoConfigurator
    {
        ...
        void CheckConfigure()
        {
            ...
            if (FrameRateDenominator.HasValue && FrameRateNumerator.HasValue)
                LoggerInstance.Debug("H264Configurator.ComputeFrameRate() {0}/{1} -> {2:F4} fps", FrameRateNumerator, FrameRateDenominator, FrameRateNumerator / (double)FrameRateDenominator);


#if DEBUG
            LoggerInstance.Debug("Configuration " + Name);
#endif
