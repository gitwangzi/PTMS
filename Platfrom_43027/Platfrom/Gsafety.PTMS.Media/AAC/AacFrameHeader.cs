...
namespace Gsafety.PTMS.Media.AAC
{
    public sealed class AacFrameHeader : IAudioFrameHeader
    {
        ...
        public bool Parse(byte[] buffer, int index, int length, bool verbose = false)
        {
            ...
            if (0 != Layer)
            {
                LoggerInstance.Debug("AacFrameHeader.Parse() unknown layer: " + Layer);
                //return false;
            }
            ...
            if (verbose)
            {
                LoggerInstance.Debug("Configuration AAC layer {0} profile \"{1}\" channels {2} sampling {3}kHz length {4} CRC {5}",
