    ...
    public sealed class SignalTask : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            catch (AggregateException ex)
            {
                if (ex.Flatten().InnerExceptions.Any(e => !(e is OperationCanceledException)))
                    LoggerInstance.Debug("SignalTask.Dispose(): " + ex.ExtendedMessage());
            }
            catch (Exception ex)
            {
                LoggerInstance.Debug("SignalTask.Dispose(): " + ex.ExtendedMessage());
        ...
        public void Fire()
        {
            ...
            lock (_lock)
            {
                ...
                task = new Task<Task>(CallHandlerAsync, _cancellationTokenSource.Token, TaskCreationOptions.None);
            ...
            {
                LoggerInstance.Debug("SignalTask.Fire() task start failed: " + ex.Message);
        ...
        async Task CallHandlerAsync()
        {
            try
            {
                for (; ; )
                {
                    ...
                    {
                        LoggerInstance.Debug("SignalTask.CallHandlerAsync() handler failed: " + ex.Message);
                    }
                    finally
                    {
#if DEBUG
            ...
            catch (Exception ex)
            {
                ...
                LoggerInstance.Debug("SignalTask.CallHandlerAsync() failed: " + ex.Message);
            }
        }
            ...
            if (isNew)
            {
                task.ContinueWith(
                    t =>
                    {
                        ...
                        if (!ok)
                            LoggerInstance.Debug("SignalTask.WaitAsync() TrySetResult failed, status: " + taskCompletionSource.Task.Status);
                    });
            }
