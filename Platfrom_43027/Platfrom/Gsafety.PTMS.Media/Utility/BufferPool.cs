...
namespace Gsafety.PTMS.Media.Utility
{
    ...
    public sealed class BufferPool : IBufferPool
    {
...
#if DEBUG
            //LoggerInstance.Debug("Allocated Buffer {0}", bufferEntry);
#endif
        ...
        {
            //LoggerInstance.Debug("Free Buffer {0}", bufferInstance);
...
#if BUFFER_POOL_STATS
            LoggerInstance.Debug("Pool clear: alloc {0} free {1} req bytes {2}, alloc bytes {3} free bytes {4}", _allocationCount, _freeCount, _requestedAllocationBytes, _actualAllocationBytes, _actualFreeBytes);
            ...
            public void Clear()
            {
                lock (_pool)
                {
                    ...
                    if (_pool.Count != _allocationTracker.Count)
                        LoggerInstance.Debug("SubPool {0}: Pool size {1} != Tracker {2}", Size, _pool.Count, _allocationTracker.Count);
#endif

                    _pool.Clear();

#if BUFFER_POOL_STATS
                    LoggerInstance.Debug("SubPool {0}: new {1} alloc {2} free {3} allocSize {4}",
