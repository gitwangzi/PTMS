...
namespace Gsafety.PTMS.Media.TransportStream.TsParser
{
    ...
    {
        private static Dictionary<uint, int> PIDCounter = new Dictionary<uint, int>();
        ...
        public bool Parse(int tsIndex, byte[] buffer, int index)
        {
            ...
            if (PIDCounter.ContainsKey(Pid))
            {
                PIDCounter[Pid] = PIDCounter[Pid] + 1;
            }
            else
            {
                PIDCounter.Add(Pid, 1);
            }

            //LoggerInstance.Debug("PID: " + Pid + "Count:" + PIDCounter[Pid]);
            ...
            if (0 != (_adaptationFieldControl & 0x2))
            {
                ...
                if (2 == _adaptationFieldControl)
                {
                    ...
                    {
                        LoggerInstance.Debug("Invalid adaptation_field_length (without payload): " + _adaptationLength);
                ...
                else if (3 == _adaptationFieldControl)
                {
                    if (_adaptationLength > PacketSize - 6)
                    {
                        ...
                        {
                            LoggerInstance.Debug("Invalid adaptation_field_length (with payload): " + _adaptationLength);
                            _adaptationLength = PacketSize - 5;
                        }
                    }
                }
