...
{
    [ExportAsViewModel(InstallationName.MaintainApplcationManagementScheduledVm)]
    public class MaintainApplicationScheduledManageViewModel : ListViewModel<MaintainApplication>
    {
        /// <summary>
        /// 初始化内容
        /// </summary>
        public MaintainApplicationScheduledManageViewModel()
            : base()
        {
            try
            {
        ...
        private void _client_UpdateMaintainApplicationCompleted(object sender, UpdateMaintainApplicationCompletedEventArgs e)
        {
            try
            {
                if (e.Error == null)
            ...
            {
                ApplicationContext.Instance.Logger.LogException("MaintainApplicationScheduledManageViewModel", ex);
            }
            finally
            {
            ...
            try
            {
                ...
                if (dialog != null)
                {
                    if (dialog.DialogResult == true)
                    {
                        ...
                        MaintainApplication ma = new MaintainApplication();
                        ma.ID = SelectedItem.ID;
                        ma.Applicant = SelectedItem.Applicant;
                        ma.VehicleID = SelectedItem.VehicleID;
                        ma.Contact = SelectedItem.Contact;
                        ma.Status = 0;
                        ma.Creator = SelectedItem.Creator;
                        ma.CreateTime = SelectedItem.CreateTime.ToUniversalTime();
                        ma.Problem = SelectedItem.Problem;
                        ma.ClientID = SelectedItem.ClientID;

                        client.UpdateMaintainApplicationAsync(ma);
                    }
                }
            }
            catch (Exception ex)
        ...
        protected override void InitPagination()
        {
            try
            {
                Data = new PagedServerCollection<MaintainApplication>((pageIndex, pageSize) =>
                {
                    ...
                    MaintainApplicationClient client = InitialClient();
                    //查询数据
                    client.GetMaintainApplicationListAsync(ApplicationContext.Instance.AuthenticationInfo.ClientID, 2, SearchByApplicant, SearchByVehicleID, SearchByContact, pageIndex, pageSize);
                });
            }
            catch (Exception ex)
            {
