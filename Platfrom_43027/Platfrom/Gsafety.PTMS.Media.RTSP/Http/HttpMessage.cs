...
using Gsafety.PTMS.Media.RTSP.Common.Extensions.Encoding;
...
namespace Gsafety.PTMS.Media.RTSP.Http
{
    ...
    /// </summary>
    public class HttpMessage : BaseDisposable, IPacket
    {
        #region Statics
        /// <summary>
        /// 0 = HeaderName
        ...
        internal protected static char[] HeaderNameValueSplit = new char[] { (char)ASCII.Colon };

        //String which is used to split Header values of the HttpMessage
        internal protected static char[] SpaceSplit = new char[] { (char)ASCII.Space };
        ...
        internal protected char[] m_EncodedLineEnds = UTF8.LineEndingCharacters,
        ...
        public Encoding HeaderEncoding
        {
            ...
            set
            {
                ...
                m_EncodedLineEnds = m_HeaderEncoding.GetChars(UTF8.LineEndingBytes);

                m_EncodedWhiteSpace = m_HeaderEncoding.GetChars(UTF8.WhiteSpaceBytes);

                m_EncodedColon = m_HeaderEncoding.GetChars(UTF8.ColonBytes);

                m_EncodedSemiColon = m_HeaderEncoding.GetChars(UTF8.SemiColonBytes);

                m_EncodedForwardSlash = m_HeaderEncoding.GetChars(UTF8.ForwardSlashBytes);
            ...
            : base(shouldDispose)
        {
            ...
            if (data == null || offset < 0 || offset >= (length = Binary.Clamp(length, 0, data.Length)))
            {
                return;
            }
        ...
        virtual protected bool ParseStatusLine(bool force = false)
        {
            ...
            string StatusLine;// = ASCII.ReadLine(m_Buffer, Encoding);
            ...
            if (false == (sawDelemit = EncodingExtensions.ReadDelimitedDataFrom(HeaderEncoding, m_Buffer, m_EncodedLineEnds, m_Buffer.Length, out StatusLine, out read, true)) && read < MinimumStatusLineSize)
            {
            ...
            string[] parts = StatusLine.Split(SpaceSplit);
            ...
            if (MessageType == HttpMessageType.Request)
            {
                ...
                if (string.IsNullOrWhiteSpace(MethodString) ||
                    ...
                    false == double.TryParse(ASCII.ExtractPrecisionNumber(part.Substring(Protocol.Length)), //Try to parse the version at the protocol length
        ...
        virtual protected bool ParseHeaders(bool force = false)
        {
            try
            {
                ...
                while (false == IsDisposed && m_Buffer.CanRead && emptyLine <= 2 && (remains = max - position) > 0)
                {
                    ...
                    sawDelemit = EncodingExtensions.ReadDelimitedDataFrom(HeaderEncoding, m_Buffer, m_EncodedLineEnds, remains, out rawLine, out justRead, out encountered, true);
                    ...
                    //E.g. Rtp-Info: Http://....
                    //parts = rawLine.Split(HeaderNameValueSplit);
                    var index = rawLine.IndexOf(HeaderNameValueSplit.FirstOrDefault());
                    if (index < 0)
                    {
                        parts = new[] { rawLine };
                    }
                    ...
                    {
                        parts = new[] { rawLine.Substring(0, index), rawLine.Substring(index + 1, rawLine.Length - index - 1) };
                    }
                    //parts = rawLine.Split(HeaderNameValueSplit);

                    //not a valid header
                    //if (parts.Length <= 1 || string.IsNullOrWhiteSpace(parts[0]))
                    if (index < 0 || string.IsNullOrWhiteSpace(parts[0]))
                    {
                        readingValue = true;
                        headerValue.Append(rawLine);
        ...
        internal protected virtual bool ParseContentLength(bool force = false)
        {
            ...
            if (false == int.TryParse(ASCII.ExtractNumber(contentLength), System.Globalization.NumberStyles.Integer, System.Globalization.CultureInfo.InvariantCulture, out m_ContentLength))
            {
                return false;
            }
        ...
        internal protected virtual Encoding ParseContentEncoding(bool force = false, bool raiseWhenNotFound = false)
        {
            ...
            if (false == string.IsNullOrWhiteSpace(contentEncoding))
            {
                ...
                contentDecoder = System.Text.Encoding.GetEncoding("contentEncoding");
                if (contentDecoder == null)
                {
                    contentDecoder = System.Text.Encoding.UTF8;
                }
            }
        ...
        public override string ToString() { return System.Text.Encoding.UTF8.GetString(System.Text.Encoding.UTF8.GetBytes(ToEncodedString())); }
        ...
        internal string GetHeaderValue(string name, out string actualName)
        {
            ...
            foreach (string headerName in GetHeaders())
                if (string.Compare(name, headerName, System.StringComparison.OrdinalIgnoreCase) == 0) //headerName.Equals(name, StringComparison.OrdinalIgnoreCase);
                {
        ...
        public bool TryGetBuffers(out System.Collections.Generic.IList<System.ArraySegment<byte>> buffer)
        {
            ...
            try
            {
                ...
                var buff = m_Buffer.GetBuffer();
                existingBuffer = new ArraySegment<byte>(buff);
        ...
        {
            if (IsDisposed && false == IsPersistent) return MemorySegment.EmptyBytes;
        ...
        {
            IEnumerable<byte> sequence = MemorySegment.Empty;
        ...
        {
            if (IsDisposed && false == IsPersistent) return MemorySegment.Empty;
        ...
        /// <returns></returns>
        public virtual async Task<int> CompleteFrom(System.Net.Sockets.Socket socket, MemorySegment buffer)
        {
            if (IsDisposed && false == IsPersistent) return 0;
        ...
        int ParseChunk(ref int offset, int length) //out string extensions
        {
            ...
            //If it was not present then do not parse further
            if (false == (sawDelemit = EncodingExtensions.ReadDelimitedDataFrom(HeaderEncoding, m_Buffer, m_EncodedLineEnds, length, out ChunkLine, out read, true)))
            {
                return -1;
            }
            ...
            string[] parts = ChunkLine.TrimEnd().Split(HttpHeaders.ValueSplit);
