...
namespace Gsafety.PTMS.Media.RTSP.Sdp
{
    public class SessionDescription : Common.BaseDisposable, IEnumerable<SessionDescriptionLine>
    {
        ...
        public static bool TryParseRange(string value, out string type, out TimeSpan start, out TimeSpan end)
        {
            ...
            type = "Unknown";
            start = TimeSpan.Zero;
            end = Gsafety.PTMS.Media.RTSP.Common.Extensions.TimeSpan.TimeSpanExtensions.InfiniteTimeSpan;
        ...
        internal protected Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionVersionLine m_SessionVersionLine;
        internal protected Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionOriginLine m_OriginatorLine;
        internal protected Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionNameLine m_NameLine;
        ...
        System.Threading.ManualResetEvent m_Update = new System.Threading.ManualResetEvent(true);
        ...
        public string OriginatorAndSessionIdentifier
        {
            ...
            set
            {
                ...
                m_OriginatorLine = new Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionOriginLine(value);
        ...
        {
            get { return false == m_Update.WaitOne(0) || m_UpdateTokenSource.IsCancellationRequested; } //When requested may already be cancelled and no longer under modification...
        }
            ...
            : base(shouldDispose)
        {
            ...
            if (lines.Length < 3) Gsafety.PTMS.Media.RTSP.Common.TaggedExceptionExtensions.RaiseTaggedException(lines, "Invalid Session Description, At least 3 lines should be found.");
            ...
            for (int lineIndex = 0, endIndex = lines.Length; lineIndex < endIndex; /*Advancement of the loop controlled by the corrsponding Lines via ref*/)
            {
                ...
                switch (line[0])
                {
                    case Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionVersionLine.VersionType:
                        {
                            m_SessionVersionLine = new Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionVersionLine(lines, ref lineIndex);
                            continue;
                        }
                    case Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionOriginLine.OriginType:
                        {
                            m_OriginatorLine = new Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionOriginLine(lines, ref lineIndex);
                            continue;
                        }
                    case Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionNameLine.NameType:
                        {
                            m_NameLine = new Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionNameLine(lines, ref lineIndex);
                            continue;
                        }
                    case Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionTimeDescriptionLine.TimeType:
                        {
                            m_TimeDescriptions.Add(new TimeDescription(lines, ref lineIndex));
                            continue;
                        }
                    case Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionMediaDescriptionLine.MediaDescriptionType:
                        {
                            m_MediaDescriptions.Add(new MediaDescription(lines, ref lineIndex));
                            continue;
                        }
                    //case Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionAttributeLine.AttributeType:
                    //    {
                    //Should check or charset or sdpland attribute and switch currentEncoding.
                    //        m_Lines.Add(new Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionAttributeLine(lines, ref lineIndex));
                    //        continue;
                    //    }
                    //case Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionBandwidthLine.BandwidthType:
                    //    {
                    //        m_Lines.Add(new Gsafety.PTMS.Media.RTSP.Sdp.Lines.SessionBandwidthLine(lines, ref lineIndex));
                    //        continue;
                    //    }
                    default:
                        {
        ...
        public System.Threading.CancellationToken BeginUpdate()
        {
            ...
            if (m_UpdateTokenSource.Token.WaitHandle.WaitOne() && m_Update.WaitOne())
            {
                m_Update.Reset();
            }
        ...
        public static bool SupportsAggregateMediaControl(this SessionDescription sdp, out Uri controlUri, Uri baseUri = null)
        {
            ...
            if (false == string.IsNullOrWhiteSpace(controlPart))
            {
                /*
                    ...
                    entire base URL.
                 */
                controlPart = controlPart.Split(Gsafety.PTMS.Media.RTSP.Sdp.SessionDescription.ColonSplit, StringSplitOptions.RemoveEmptyEntries).Last();
