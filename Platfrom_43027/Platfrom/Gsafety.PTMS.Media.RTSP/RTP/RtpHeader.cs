...
namespace Gsafety.PTMS.Media.RTSP.RTP
{
     ...
     ******  (Subsequently by the Maximum Transmittable Unit if applicable of the underlying network which implements the underlying protocol)  ******
        ...
        /// </summary>
        internal Gsafety.PTMS.Media.RTSP.RTP.RFC3550.CommonHeaderBits First16Bits;
            ...
            : base(shouldDispose)
        {
            ...
            if (octetsLength > 2)
            {
                ...
                //SSRC (4 octets / U32)
                Array.Copy(octets, offset + 2, Last10Bytes, 0, (int)(Math.Min(10, octetsLength - 2)));
            }
        ...
        public RtpHeader(Common.MemorySegment memory, bool shouldDispose = true)
            : base(shouldDispose)
        {
            First16Bits = new Gsafety.PTMS.Media.RTSP.RTP.RFC3550.CommonHeaderBits(memory, shouldDispose);

            SegmentToLast10Bytes = new Common.MemorySegment(memory.Array, memory.Offset + Gsafety.PTMS.Media.RTSP.RTP.RFC3550.CommonHeaderBits.Size, Binary.Clamp(memory.Count - RFC3550.CommonHeaderBits.Size, 0, 10), shouldDispose);
        }
        ...
        public RtpHeader(int version, bool padding, bool extension, bool shouldDispose = true)
            : base(shouldDispose)
        {
            First16Bits = new Gsafety.PTMS.Media.RTSP.RTP.RFC3550.CommonHeaderBits(version, padding, extension);
