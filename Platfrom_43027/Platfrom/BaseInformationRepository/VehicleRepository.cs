...
namespace Gsafety.PTMS.BaseInformation.Repository
{
    public class VehicleRepository : Gsafety.PTMS.BaseInfo.BaseRepository
    {
        ...
        public static SingleMessage<bool> InsertVehicle(PTMSEntities context, Vehicle vehicle)
        {
            ...
            exist = context.BSC_VEHICLE.Where(x => x.FICHA  == vehicle.Ficha && x.VALID == 1 && x.CLIENT_ID == vehicle.ClientId).Any();
            if (exist)
            {
                return new SingleMessage<bool>(false, "FichaDuplicate");
            }
        ...
        public MultiMessage<Vehicle> CheckVehicleExist(List<Vehicle> vehicleList)
        {
            ...
            {
                //Batch Check
                Func<Vehicle, bool> filter = tv => context.BSC_VEHICLE.Local.Any(v => v.VALID == 1 && (v.VEHICLE_SN == tv.VehicleSn || v.VEHICLE_ID == tv.VehicleId || v.ENGINE_ID == tv.EngineId||v.FICHA == tv.Ficha))
                     || context.BSC_VEHICLE.Any(v => v.VALID == 1 && (v.VEHICLE_SN == tv.VehicleSn || v.VEHICLE_ID == tv.VehicleId || v.ENGINE_ID == tv.EngineId || v.FICHA == tv.Ficha));
        ...
        public bool BatchAdd(List<Vehicle> vehicleList)
        {
            using (PTMSEntities context = new PTMSEntities())
            {
                ...
                var result = context.BSC_VEHICLE.Where(x => x.VALID == 0 && (idList.Contains(x.VEHICLE_ID) || snList.Contains(x.VEHICLE_SN)||idList.Contains(x.FICHA))).ToList();
                ...
                for (int i = 0; i < vehicleList.Count; i++)
                {
                    ...
                    //Batch Delete 
                    var result1 = context.BSC_VEHICLE.Where(x => (x.VEHICLE_ID == item.VehicleId || x.FICHA == item.Ficha) && x.VALID == 0).FirstOrDefault();
                    if (result1 != null)
                    {
