...
{
    public class LogManageService : BaseService, IAlarmDealLogService, ICarAlertDealLogService, IInstallLogService, ILoginLogService, IVideoDowmloadLogService, IVideoLogSerVice, IVisitLogService, IGetSuiteInfoService, ILogOperate, ILogVideo, IPTMSLogError
    {
       ...
       //public  SingleMessage<bool> InsertErrorLog(LogError model)
       // {
       //     SingleMessage<bool> result = null;

       //     using (var context = new PTMSEntities())
       //     {
       //         result = LogErrorRepository.InsertLogError(context, model);
       //     }

       //     return result;
       // }

      public   MultiMessage<LogError> GetErrorLog(string clientID, string content, DateTime? startTime, DateTime? endTime, PagingInfo pageInfo)
        {
            try
            {
                Info("GetErrorLog");
                Info("clientID:" + Convert.ToString(clientID) + ";" + "content:" + Convert.ToString(content) + ";"  + "startTime:" + Convert.ToString(startTime) + ";" + "endTime:" + Convert.ToString(endTime) + ";" );

                MultiMessage<LogError> result = null;
                using (PTMSEntities context = new PTMSEntities())
                {
                    result = LogErrorRepository.GetLogErrorList(context,pageInfo.PageIndex,pageInfo.PageSize,content, startTime, endTime);
                }
            ...
            {
                Error(ex);
                return new MultiMessage<LogError>() { Result = null, ExceptionMessage = ex };
            }
        }
