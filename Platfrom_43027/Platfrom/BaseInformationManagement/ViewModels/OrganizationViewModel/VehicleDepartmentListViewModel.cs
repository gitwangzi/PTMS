    ...
    public class VehicleDepartmentListViewModel : UploadAndExportViewModelBase<Vehicle>
    {
        ...
        public VehicleDepartmentListViewModel()
        {
            try
            {
                ...
                BtnCopyCommand = new ActionCommand<object>(obj => Update("copy"));
                    ...
                    client.GetBscVehicleListCompleted += (s, e) =>
                    {
                        try
                        {
                            if (e.Result != null && e.Result.TotalRecord > 0)
                            {
                                ...
                                Names.Add(ApplicationContext.Instance.StringResourceReader.GetString("Ficha"));
                                Names.Add(ApplicationContext.Instance.StringResourceReader.GetString("ProductDate"));
                                ...
                                foreach (var item in e.Result.Result)
                                {
                                    try
                                    {
                                        ...
                                        vem.Note = item.Note;
                                        vem.Ficha = item.Ficha;
                                        vem.ProductDate = item.ProductDate;
                                        list.Add(vem);
                                    }
                                    catch (Exception)
                                    {
        ...
        private List<Vehicle> GetUploadList()
        {
            ...
            for (int i = startIndex; i < endIndex; i++)
            {
                ...
                tempVehicle.Ficha = uploadContent[i][10].ToString().Trim();
                if (tempVehicle.Ficha == string.Empty)
                {
                    errorIndex = i;
                    errorCode.Append("K" + (i + 1).ToString() + ApplicationContext.Instance.StringResourceReader.GetString("Info_isEmpty"));
                    errorCode.Append(";");
                }
                if (tempVehicle.Ficha.Trim().Length > 20)
                {
                    errorIndex = i;
                    errorCode.Append("K" + (i + 1).ToString() + "  " + ApplicationContext.Instance.StringResourceReader.GetString("Length_Warning"));
                    errorCode.Append(";");
                }
                if (tempVehicleList.Any(t => t.Ficha == tempVehicle.Ficha.Trim()))
                {
                    errorIndex = i;
                    errorCode.Append("K" + (i + 1).ToString() + "  " + ApplicationContext.Instance.StringResourceReader.GetString("ValueTheSame"));
                    errorCode.Append(";");
                }

                var days = 0;
                if (uploadContent[i][11].ToString().Trim() != string.Empty)
                {
                    if (!int.TryParse(uploadContent[i][11].ToString().Trim(), out days))
                    {
                        errorIndex = i;
                        errorCode.Append(ConvertColumnIndexToStr(11) + (i + 1).ToString() +
                    ...
                    tempVehicle.ProductDate = dt.ToUniversalTime();
                }
                else
                {
                    tempVehicle.ProductDate = null;
                }

                tempVehicle.OperationLicense = uploadContent[i][12].ToString().Trim();
                ...
                tempVehicle.Owner = uploadContent[i][13].ToString().Trim();
                ...
                tempVehicle.Contact = uploadContent[i][14].ToString().Trim();
                ...
                tempVehicle.ContactPhone = uploadContent[i][15].ToString().Trim();
                ...
                if (!ValidatePhone(uploadContent[i][15].ToString().Trim()))
                {
                    errorIndex = i;
                    errorCode.Append("P" + (i + 1).ToString() +
                                     ApplicationContext.Instance.StringResourceReader.GetString(
                                         "Phone_Illegal"));
                    errorCode.Append(";");
                }

                tempVehicle.ContactEmail = uploadContent[i][16].ToString().Trim();
                if (ValidateEmail(tempVehicle.ContactEmail))
                {
                    errorIndex = i;
                    errorCode.Append("Q" + (i + 1).ToString() + "  " + ApplicationContext.Instance.StringResourceReader.GetString("BASEINFO_EMail") + "\n" + ApplicationContext.Instance.StringResourceReader.GetString("BASEINFO_FormatIllegal"));
                    errorCode.Append(";");
                }
                if (tempVehicle.ContactEmail.Trim().Length > 100)
                {
                    errorIndex = i;
                    errorCode.Append("Q" + (i + 1).ToString() + "  " + ApplicationContext.Instance.StringResourceReader.GetString("Length_Warning"));
                    errorCode.Append(";");
                }

                tempVehicle.ContactAddress = uploadContent[i][17].ToString().Trim();
                if (tempVehicle.ContactAddress.Trim().Length > 512)
                {
                    errorIndex = i;
                    errorCode.Append("R" + (i + 1).ToString() + "  " + ApplicationContext.Instance.StringResourceReader.GetString("Length_Warning"));
                    errorCode.Append(";");
                }
            ...
            try
            {
                ...
                else
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        errorCode.Append(" ");
                        errorCode.Append(ApplicationContext.Instance.StringResourceReader.GetString("Ficha"));
                        errorCode.Append(item.Ficha);
