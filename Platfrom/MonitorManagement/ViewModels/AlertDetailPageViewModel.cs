...
{
    [ExportAsViewModel(MonitorName.MonitorAlertInfoViewModel)]
    public class AlertDetailPageViewModel : BaseViewModel, IEventSink<MonitorAlertInfoDisplay>, IEventSink<int>, IPartImportsSatisfiedNotification
    {
        ...
        public void HandleEvent(MonitorAlertInfoDisplay alertinfo)
        {
            try
            {
                ...
                SuiteID = alertinfo.DisPlayInfo.SuiteID;
                ...
                if (alertinfo.DisPlayInfo != null)
                {
                    if (this.client == null)
                    {
                        this.InilitClient();
                    }
                    this.client.GetChauffeurByVehicleAsync(alertinfo.DisPlayInfo.VehicleId, ApplicationContext.Instance.AuthenticationInfo.ClientID);
                    this._vehicleAlertClient.GetVehicleAlertDisposeInfoAsync(alertinfo.DisPlayInfo.Id, alertinfo.DisPlayInfo.VehicleId, ApplicationContext.Instance.AuthenticationInfo.ClientID);
                    if (string.IsNullOrEmpty(alertinfo.DisPlayInfo.Province) == false && string.IsNullOrEmpty(alertinfo.DisPlayInfo.City) == false)
                    {
                        Province = alertinfo.DisPlayInfo.Province;
                        City = alertinfo.DisPlayInfo.City;
                        this.DistrictCodeDescribe = Province + "/" + City;
                    }
                    else if (!string.IsNullOrEmpty(alertinfo.DisPlayInfo.DistrictCode))
                    {

                        if (alertinfo.DisPlayInfo.DistrictCode.Length == 5)
                        {

                            string provicecode = alertinfo.DisPlayInfo.DistrictCode.Substring(0, 2);
                            ...
                            var city = ApplicationContext.Instance.BufferManager.DistrictManager.Cities.FirstOrDefault(n => n.Code == alertinfo.DisPlayInfo.DistrictCode);
                            if (city != null)
                            {
                                City = city.Name;
                            }
