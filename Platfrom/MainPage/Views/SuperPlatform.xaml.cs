...
{
    [ExportAsView(MainPageName.SuperPlatformV)]
    [ExportViewToRegion(MainPageName.SuperPlatformV, ViewContainer.LoginContainer)]
    public partial class SuperPlatform : UserControl
    {
        ...
        private void SuperFrame_Navigated(object sender, System.Windows.Navigation.NavigationEventArgs e)
        {

        }

        private void NativeAccordion_OnSelectedItemsChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            var accordion = sender as Accordion;
            if (accordion != null)
            {
                ...
                if (accordion.SelectedIndex == 0)
                {
                    ...
                    {
                        this.SuperFrame.Source = new Uri("/OrderClientManageV", UriKind.RelativeOrAbsolute);
                    }
                }
                else if (accordion.SelectedIndex == 1)
                {
                    if (this.SuperFrame != null)
                    {
                        this.SuperFrame.Source = new Uri("/ManageLogV", UriKind.RelativeOrAbsolute);
                    }
                }
            }
        }
        string HelpDir = "/Super/index.html";
